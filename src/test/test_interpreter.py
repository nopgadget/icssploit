...
from src.exploits import Exploit, Option, GLOBAL_OPTS
...
class icssploitInterpreterTest(icssploitTestCase):
    ...
    def setUp(self):
        ...
        self.raw_prompt_default = "\001\033[4m\002icssploit\001\033[0m\002 > "
        self.module_prompt_default = lambda x: "\001\033[4m\002icssploit\001\033[0m\002 (\001\033[91m\002{}\001\033[0m\002) > ".format(x)
        GLOBAL_OPTS.clear()

    def prepare_prompt_env_variables(self, raw_prompt=None, module_prompt=None):
        if raw_prompt:
            os.environ["ICSSPLOIT_RAW_PROMPT"] = raw_prompt
        else:
            try:
                os.environ["ICSSPLOIT_RAW_PROMPT"]
            except KeyError:
                pass

        if module_prompt:
            os.environ["ICSSPLOIT_MODULE_PROMPT"] = module_prompt
        else:
            try:
                del os.environ["ICSSPLOIT_MODULE_PROMPT"]
            except KeyError:
                pass
    ...
    def test_custom_raw_prompt(self):
        ...
        self.assertEqual("***icssploit***", self.interpreter.prompt)
    ...
    def test_custom_module_prompt(self):
        ...
        self.assertEqual("*icssploit*{} >>>".format(module_name), self.interpreter.prompt)
